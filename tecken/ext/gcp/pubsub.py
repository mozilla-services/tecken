...
class GoogleNotificationQueue(NotificationQueue):
    ...
    :arg project_id: The id of the GCP project id hosting the pub/sub subscription.
    :arg subscription_name: The resource name of the pub/sub subscription.
    """
...
def _subscription_callback(process: Callable[[Notification], None], message: Message):
    ...
    message being nacked, so we will receive it again, and it's pointless to reprocess a message
    causing a permanent error, i.e. an error that will definitely occur again next time. (The
    pub/sub queue is configured with a maximum number of delivery attempts, so permament errors
    won't cause infinite loops, just unnecessary reprocessing.)
    """
    if message.attributes.get("eventType") != "OBJECT_FINALIZE":
        # The notifications should be configured to only send messages for events of this type,
        # but it doesn't hurt double checking.
        message.ack()
        return
    ...
    try:
        ...
        event_time = datetime.datetime.fromisoformat(message.attributes["eventTime"])
        ...
        if last_modified is not None:
            metadata.last_modified = datetime.datetime.fromisoformat(last_modified)
        ...
        for backend in storage.get_download_backends(try_symbols):
            if (
                ...
                try:
                    ...
                    # Exceptions during this call are potentially temporary, so we should nack the
                    # message and bail out.
                    logger.exception("exception while building download URL")
                    message.nack()
                    return
                # We need to include the object's generation in the URL to avoid potential race
                # conditions when the same file is uploaded multiple times in rapid succession.
        ...
        # The message somehow could not be parsed. It's body isn't valid JSON, it's missing a
        # required attribute or similar. It doesn't make sense to retry this message – the parse
        # error won't go away next time. So we simply ack the message and log the exception.
        logger.exception("error extracting data from pub/sub message")
        message.ack()
        return
    notification = Notification(
        event_type=event_type, event_time=event_time, key=key, metadata=metadata
    )
    try:
