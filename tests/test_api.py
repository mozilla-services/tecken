...
def test_users(client):
    ...
    Upload.objects.create(user=user, size=1234)
...
def test_edit_user(client):
    ...
    response = client.post(
        url, {"groups": "JUNK", "is_active": False, "is_superuser": True}
    )
    assert response.status_code == 400

    response = client.post(
        url, {"groups": new_group.id, "is_active": False, "is_superuser": True}
    )
    assert response.status_code == 200
...
def test_edit_user_permissions(client):
    ...
    response = client.post(
        url, {"is_active": True, "groups": Group.objects.get(name="Uploaders").id}
    )
    assert response.status_code == 200
    # assert user.groups.all().count() == 1
    ...
    response = client.post(
        url, {"is_active": True, "groups": Group.objects.get(name="Upload Auditors").id}
    )
    assert response.status_code == 200
...
def test_current_settings(client, settings):
    ...
    response = client.get(url)
    assert response.status_code == 200
    current_settings = {x["key"]: x["value"] for x in response.json()["settings"]}
...
def test_current_versions(client):
    ...
    response = client.get(url)
    assert response.status_code == 200
    current_versions = {x["key"]: x["value"] for x in response.json()["versions"]}
